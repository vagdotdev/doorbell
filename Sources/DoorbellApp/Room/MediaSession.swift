...
class MediaSession: ObservableObject {
    ...
    init() {
        ...
        gainSink = IncomingAudio.shared.$gain.sink { [weak self] gain in self?.applyVolume(Double(gain)) }
    }
    ...
    func connect(_ grant: MediaGrant, microphone: Bool, camera: Bool) async throws {
        ...
        try await enqueue { [self] in
            ...
            do {
                ...
                try generation.check(ticket)
                if microphone {
                    do { _ = try await room.localParticipant.setMicrophone(enabled: true) }
                    catch { problem = "Microphone unavailable. Check macOS permissions or choose another input." }
                    try generation.check(ticket)
                }
                if camera {
                    do { _ = try await room.localParticipant.setCamera(enabled: true) }
                    catch { problem = "Camera unavailable. You can keep talking without video." }
                    try generation.check(ticket)
                }
                phase = .connected
                refresh()
    ...
    func setMicrophone(_ on: Bool) async {
        await update("Couldn’t change the microphone. Check macOS permissions.") { [self] in
            _ = try await room.localParticipant.setMicrophone(enabled: on)
        }
    }
    ...
    private func update(_ message: String, action: @escaping @MainActor () async throws -> Void) async {
        guard isConnected, !isUpdating else { return }
        isUpdating = true
        ...
        do {
            ...
            if generation.isCurrent(ticket) { problem = message; refresh() }
        }
    }
